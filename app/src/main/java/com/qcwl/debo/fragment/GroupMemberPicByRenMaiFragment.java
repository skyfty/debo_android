...
public class GroupMemberPicByRenMaiFragment extends Fragment implements ContactListPresenterInf {
    private View view;
    ...
    public void onResume() {
        super.onResume();
        ConversationSqlite sqlite = ConversationSqlite.getInstance(getActivity());
        SQLiteDatabase db = sqlite.getReadableDatabase();
        SourceDateList.clear();
        cursor_list.clear();
        Cursor cursor = db.rawQuery("select * from renmai", null);
    ...
    private List<ContactsBean> filledData(List<ContactsBean> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (TextUtils.isEmpty(list.get(i).getUser_nickname())) {
                // 汉字转换成拼音
                pinyin = characterParser.getSelling(list.get(i).getMobile());
            } else {
                sortModel.setUser_nickname(list.get(i).getUser_nickname());
                // 汉字转换成拼音
                pinyin = characterParser.getSelling(list.get(i).getUser_nickname());
