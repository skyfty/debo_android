...
import com.qcwl.debo.widget.InviteMessage.InviteMesageStatus;
...
public class NewFriendsMsgAdapter extends ArrayAdapter<InviteMessage> {
    ...
    private SPUtil sp;
    private String TAG = "NewFriendsMsgAdapter";
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (msg != null) {
            ...
            if (msg.getGroupId() == null)
                EaseUserUtils.setUserNick(context, msg.getFrom(), holder.name, holder.avator);
            ...
            if (msg.getStatus() == InviteMesageStatus.BEAGREED) {
                ...
                holder.agree
                        .setBackgroundResource(R.drawable.rect_green);
                holder.agree.setText(str2);

                holder.status.setVisibility(View.VISIBLE);
                holder.status.setEnabled(true);
                holder.status
                        .setBackgroundResource(R.drawable.rect_red3);
                ...
                } else if (msg.getStatus() == InviteMesageStatus.BEAPPLYED) { // application
                    // to
                    // join
                    // group
                ...
                holder.agree.setOnClickListener(new OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        if (NetWorkUtils.isConnected(context)) {
                            Api.addFriends(sp.getString("uid"), msg.getFrom(), new ApiResponseHandler(context) {
                                @Override
    ...
    private void acceptInvitation(final Button buttonAgree,
        ...
        new Thread(new Runnable() {
            public void run() {
                // call api
                try {
                    if (msg.getStatus() == InviteMesageStatus.BEINVITEED) {// accept
                        // be
                        // friends
                        EMClient.getInstance().contactManager()
                                .acceptInvitation(msg.getFrom());
                    } else if (msg.getStatus() == InviteMesageStatus.BEAPPLYED) { // accept
                        // application
                        // to
                        // join
                        // group
    ...
    private void refuseInvitation(final Button buttonAgree,
        ...
        new Thread(new Runnable() {
            public void run() {
                // call api
                try {
                    if (msg.getStatus() == InviteMesageStatus.BEINVITEED) {// decline
                        // the
                        // invitation
                        EMClient.getInstance().contactManager()
                                .declineInvitation(msg.getFrom());
                    } else if (msg.getStatus() == InviteMesageStatus.BEAPPLYED) { // decline
                        // application
                        // to
                        // join
                        // group
                    ...
                    ((Activity) context).runOnUiThread(new Runnable() {
                        ...
                        @Override
                        public void run() {
                            Log.i(TAG,"...........refuseInvitation="+str2);
                            pd.dismiss();
