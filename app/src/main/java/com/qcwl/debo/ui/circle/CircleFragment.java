...
//    int top, alpha;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_circle3, container, false);
...
//                swipeToLoadLayout.setSwipeStyle(SwipeToLoadLayout.);
        ...
        listView.setOnScrollListener(new AbsListView.OnScrollListener() {
            ...
            public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
                ...
                int head = dp2px(250);
                int head2 = dp2px(70);
                Log.i(TAG,".......onScroll="+top+"     ="+statusHeight+"    "+dp2px(statusHeight)+"     ="+head+"     ="+(head-top)+"      ="+head2);
                if (head - top < 50){
                    if (scallerValue2 > scallerValue) {
                        scallerValue = head - top;
                    }
                }else if(head -top <0){
                    //改变title背景颜色
                    Log.i(TAG,".......onScroll=");
                    layoutTitle.setBackgroundColor(Color.parseColor("#71cca2"));
                }
//                top = getScrollY();// headerView.getTop();
//                if (Math.abs(top) < dp2px(200) - spacing) {
//                    alpha = (int) (1.0f * Math.abs(top) / (dp2px(200) - spacing) * 255);
//                    tabLayout.setSelectedTabIndicatorHeight(dp2px(0));
...
//                layoutTitle.getBackground().setAlpha(alpha);
    ...
    private void showPop() {
       ...
       // mPopupWindow.showAsDropDown(tv_select);
        mPopupWindow.showAsDropDown(tv_select, -50, 0);
        // 设置pop关闭监听，用于改变背景透明度
