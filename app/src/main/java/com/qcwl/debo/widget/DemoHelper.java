...
import com.qcwl.debo.utils.FilterStringUtil;
...
//        options.setHuaweiPushAppId("10492024");
            ...
            public Intent getLaunchIntent(EMMessage message) {
                ...
                if (isVideoCalling) {
                    ...
                    if (chatType == ChatType.Chat) { // single chat message
                        ...
                        try {
                            intent.putExtra("nickname", message.getStringAttribute("user_nickname"));
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
    ...
    protected void onUserException(String exception) {
        ...
        Log.i("异地邓肯","exception    +onUserException"+exception);
            ...
            public void onCmdMessageReceived(List<EMMessage> messages) {
                for (EMMessage message : messages) {
                    ...
                    if (action.equals("comment")) {
                        String history_list = SPUtil.getInstance(appContext).getString("comment");
                        List<String> list = new ArrayList<>();
                        list.add(message.getFrom());
                        if (!TextUtils.isEmpty(history_list)) {
                            list.add(history_list);
                        }
                        SPUtil.getInstance(appContext).setString("comment", FilterStringUtil.removeDuplicate(list));
                    }
