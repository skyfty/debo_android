...
package com.qcwl.debo.zxing.camera;
...
//	private static final int MAX_FRAME_HEIGHT = 675; // = 5/8 * 1080
	...
	public synchronized void closeDriver() {
		if (camera != null) {
			camera.release();
			camera = null;
			// Make sure to clear these each time we joke_close the camera, so that
			// any scanning rect
...
//					MIN_FRAME_HEIGHT, MAX_FRAME_HEIGHT);
			...
			int width = findDesiredDimensionInRange(screenResolution.x,
					MIN_FRAME_WIDTH, MAX_FRAME_WIDTH)*11/10;
			int height = findDesiredDimensionInRange(screenResolution.y,
					MIN_FRAME_HEIGHT, MAX_FRAME_HEIGHT)*11/10;
